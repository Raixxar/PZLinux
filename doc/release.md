...
- Package objectives use a configurable 5-tile interaction zone and can recover legacy client state when no other location-based contract type is active; rejected menu conditions emit a compact diagnostic line to `console.txt`.
- Contract items deposited through a mailbox are now removed authoritatively by the server and the removal is explicitly synchronized to the owning client, preventing delivered packages from remaining visible in the inventory in hosted multiplayer.
- Contract offers are now consumed globally and persisted as soon as the server accepts them; they cannot be repeated after completion or cancellation and remain unavailable to every player until the next weekly server board generation. Existing world contracts from the current board cycle are pruned from legacy boards automatically.
...
- Zombie Race runners now use a dedicated green zombie pictogram matching the card-suit icon style; during a race, each icon moves across its lane instead of rendering an increasingly long string of progress dots.
