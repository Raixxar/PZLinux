...
[h3]Why was my deposit refused with an "ATM full" message ?[/h3]

Each ATM only holds a finite amount of cash, capped at $50,000, and only slowly restocks itself over time instead of holding unlimited money. If a deposit would push it past that limit, it's refused so you don't lose your cash — it's not gone, it's simply still in your inventory. Withdraw from another ATM, wait for it to restock, or try again later.

[h3]I'm sitting on a huge pile of cash from looting and every ATM I try is full. What do I do ?[/h3]

Take the "Refill an ATM" contract. The moment a fresh instance of it targets an ATM, that machine's own cash reserve resets down to a small amount — even if it had already filled all the way up to $50,000. That reset alone opens up nearly $50,000 of fresh room at that one ATM, which you can deposit into normally (not just through the contract's own transaction), so you always have somewhere to unload a big stash. It never touches your bank balance directly — it only clears space in that ATM's own reserve.
...
You can only hold one contract at a time. If you find this contract too difficult, go to the contract interface to cancel it. Cancelling costs you reputation, so it isn't free — see the Reputation section below.
...
Completing a contract or a mail mission raises your reputation. Cancelling a contract lowers it. Left alone, reputation slowly drifts back toward neutral over time instead of staying at an extreme forever.
...
Open the Online Betting submenu from the computer to reach the betting hub, where you can choose between Zombie Races, Blackjack and Poker. All bets and payouts are deducted from and paid into your bank account, so make sure you have money deposited first.

[h3]Zombie Races ?[/h3]

Bet on a runner before the race starts and watch it play out. The payout is settled once the race actually finishes, not the moment it starts.
...
Yes. PZLinux keeps the same mod ID as earlier versions and stores its data in persistent ModData attached to your characters and the world, so it's safe to add to an existing save.

[h3]Do I need any other mod for this to work ?[/h3]
