...
local transferPosition = assert(sharedSource:find("function PZLinuxHackingTransfer", autoPosition, true), "Hacking transfer function not found")
local autoFunction = sharedSource:sub(autoPosition, transferPosition - 1)
...
assert(autoFunction:find("ZombRand(100, 201)", 1, true), "automatic Hacking must pay the reduced 1.0.18 amount")
assert(not autoFunction:find("ZombRand(300, 501)", 1, true), "automatic Hacking must not keep the old high payout")
